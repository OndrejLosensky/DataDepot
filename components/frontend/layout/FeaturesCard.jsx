...
const FeaturesCard = ({ heading, description, smallDescription, glow }) => {
  ...
  const bgStyle = isHovered ? { background: `${glow}` } : { background: `${glow}`};
  


  return (
    <div className="features-card w-[400px]" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} style={boxShadowStyle}>
      <div className={`w-14 mb-2 text-center px-2 py-1 rounded-xl text-white text-xs font-semibold tracking-wide`} style={bgStyle}>
        New
      </div>
